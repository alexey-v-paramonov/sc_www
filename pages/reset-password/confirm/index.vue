      ...
      <v-container>
        <v-row no-gutters md="6">
          ...
          <v-col md="6">
            ...
            <!-- Confirm form -->
            <v-form v-if="!resetConfirmDone && !badToken" @submit.prevent="resetConfirmPassword" :disabled="isPassResetConfirmSubmitting">
              ...
              <v-btn type="submit" :disabled="isPassResetConfirmSubmitting" block class="mt-2">{{ isPassResetConfirmSubmitting ? $t('loading')  : $t('password_reset.cta') }}</v-btn>
            </v-form>

            <div class="mt-2" v-if="badToken">
              <v-card
                  :title="$t('password_reset.token_is_invalid')"
                  :text="$t('password_reset.token_is_invalid_text')"
            ...
            <div class="mt-2" v-if="resetConfirmDone">
              <v-card
                :title="$t('password_reset.done_title')"
                :text="$t('password_reset.done_text')"
