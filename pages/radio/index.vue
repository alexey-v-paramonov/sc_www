          ...
          <tbody>
            <tr v-if="hosted_radios && hosted_radios.length > 0 && !hosted_radios_loading" v-for="item in hosted_radios" :key="item.name">
              <td>
                ...
                <div v-if="item.status == RADIO_STATUS.READY || item.status == RADIO_STATUS.BEING_CREATED || item.status == RADIO_STATUS.PENDING"><strong>{{ $t('radios.hosted.username') }}</strong>: {{ item.login }}</div>
